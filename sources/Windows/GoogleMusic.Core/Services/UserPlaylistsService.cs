    ...
    public class UserPlaylistsService : AllAccessServiceBase, IUserPlaylistsService
    {
        ...
        public async Task<UserPlaylist> CreateAsync(string name)
        {
            ...
            if (resp != null && resp.MutateResponse != null && resp.MutateResponse.Length == 1 && string.Equals(resp.MutateResponse[0].ResponseCode, "OK", StringComparison.OrdinalIgnoreCase))
            {
                ...
                var userPlaylist = new UserPlaylist { PlaylistId = mutation.Id, Title = name, TitleNorm = name.Normalize(), Type = "USER_GENERATED", CreationDate = DateTime.UtcNow, LastModified = DateTime.UtcNow };
