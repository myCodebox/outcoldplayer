            ...
            if (isFirstTimeActivated)
            {
                ...
                /*Container.Resolve<IEventAggregator>().GetEvent<ApplicationInitializedEvent>().Subscribe(
                    async (e) =>
                    {
                        ...
                        await dispatcher.RunAsync(() => Container.Resolve<IMainFrameRegionProvider>()
                            .SetContent(MainFrameRegion.Links, ApplicationBase.Container.Resolve<LinksRegionView>()));
