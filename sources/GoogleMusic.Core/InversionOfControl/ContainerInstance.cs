...
namespace OutcoldSolutions.GoogleMusic.InversionOfControl
{
    using System;
    ...
    internal class ContainerInstance : IContainerInstance, IDisposable
    {
        ...
        public IContainerInstance InjectionRule(Type type, Type typeImplementation)
        {
            ...
            if (!type.GetTypeInfo().IsAssignableFrom(typeImplementation.GetTypeInfo()))
            {
                throw new ArgumentException(
                    string.Format(
                        ...
                        "Cannot set type '{0}' as implementation because the registered type '{1}' cannot be assignable from it.",
                        typeImplementation,
                        type),
                    "typeImplementation");
            }
            ...
            if (this.injectionRules.ContainsKey(type))
            {
                throw new ArgumentException(
                    string.Format(
                        CultureInfo.CurrentCulture,
                        "Current registration already has a rule for type '{0}'.",
                        type),
                    "type");
            }
        ...
        public void As(Type typeImplementation)
        {
            ...
            if (typeInfo.IsAbstract || typeInfo.IsInterface)
            {
                throw new ArgumentException(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            "Cannot set interface or abstract class '{0}' as an implementation type.",
                            this.implementation));
            }
        ...
        public void AsSingleton()
        {
            lock (this.typeLocker)
            {
                ...
                {
                    throw new NotSupportedException("Current registration has more than one type. Specify which type you want to use for resolving.");
                }
        ...
        internal object Resolve(object[] arguments = null)
        {
            ...
            try
            {
                ...
                {
                    throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "Cannot find constructor for type '{0}'. If type has more than one constructor - use InjectAttribute to mark the constructor which you want to use for injection.", this.implementation));
                }
                                ...
                                if (value == null)
                                {
                                    ...
                                    if (p.ContainerInstance == null)
                                    {
                                        throw new InvalidOperationException(
                                            string.Format(
                                                CultureInfo.CurrentCulture, 
                                                "Cannot resolve parameter type {0}.",
                                                p.Type));
                                    }
        ...
        private void Compile()
        {
            if (this.factory == null && this.instance == null)
            {
                if (this.implementation == null)
                {
                    if (this.registeredTypes.Count > 1)
                    {
                        throw new NotSupportedException("If container instance has more than one registered type it should has the specified implementation type.");
                    }
                    ...
                    if (typeInfo.IsAbstract || typeInfo.IsInterface)
                    {
                        throw new NotSupportedException(
                            string.Format(
                                CultureInfo.CurrentCulture,
                                "Implementation is not specified for interface or abstract class '{0}'.",
                                this.implementation));
                    }
                }
        ...
        private void CheckBehavior()
        {
            ...
            {
                throw new NotSupportedException("Cannot set more than one behavior for registration.");
            }
        }
