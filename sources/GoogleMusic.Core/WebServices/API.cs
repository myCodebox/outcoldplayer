...
namespace OutcoldSolutions.GoogleMusic.WebServices
{
    ...
    public class API
    {
        ...
        private void TrackListChunkRecv(HttpWebRequest request, HttpWebResponse response, String jsonData, Exception error)
        {
            ...
            GoogleMusicPlaylist chunk = JsonConvert.DeserializeObject<GoogleMusicPlaylist>(jsonData);

            trackContainer.AddRange(chunk.Playlist);

            if (!String.IsNullOrEmpty(chunk.ContinuationToken))
            {
                GetAllSongs(chunk.ContinuationToken);
            }
            else
            {
